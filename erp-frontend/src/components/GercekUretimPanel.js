...
export default function GercekUretimPanel() {
  ...
  const selected = useMemo(
    () => recipes.find(r => String(r.ReceteId) === String(recipeId)),
    [recipes, recipeId]
  );
  ...
  const produce = async () => {
    ...
    setResult(null);
    try {
      const { data } = await axios.post(`${API_URL}/recete-uretim/${recipeId}/uret`, {
        miktar: q,
        notlar: notlar || null,
        depo
      });
  ...
  return (
    <>
      <button
        ...
        style={{
          position: 'fixed', right: 24, bottom: 88, zIndex: 2100,
      ...
      {open && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 3000, background: 'rgba(15,23,42,.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ width: 'min(620px, 100%)', background: '#fff', borderRadius: 18, padding: 24, boxShadow: '0 24px 70px rgba(0,0,0,.3)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
              <div>
                <div style={{ fontSize: 12, fontWeight: 800, color: '#64748b', letterSpacing: 1 }}>ÜRETİM İŞLEMİ</div>
                <h2 style={{ margin: '4px 0 0' }}>Gerçek Üretim</h2>
              </div>
            ...
            {selected && <div style={{ marginTop: 14, padding: 12, borderRadius: 10, background: '#f8fafc', color: '#475569' }}><b>{selected.MamulAdi}</b> üretilecek. Üretim sırasında yeterli hammadde kontrolü yapılır; işlem tek transaction içinde stok çıkışı + mamul girişi olarak tamamlanır.</div>}

            {result && <div style={{ marginTop: 14, padding: 12, borderRadius: 10, background: '#ecfdf5', color: '#166534' }}>Üretim #{result.uretimId} tamamlandı. {result.tuketi lenHammaddeler?.length || 0} hammadde hareketi işlendi.</div>}
