...
const CompanySelector = () => {
  ...
  useEffect(() => {
    const loadCompanies = async () => {
      try {
        const response = await axios.get(`${API_URL}/core/companies`);
        const list = Array.isArray(response.data) ? response.data : [];
        setCompanies(list);
        const saved = Number(localStorage.getItem(STORAGE_KEY) || 1);
        const valid = list.some(
          (company) => Number(company.CompanyId) === saved,
        );
        ...
        console.error("Yetkili şirketler alınamadı:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const selectedCompany = companies.find(
    (company) => Number(company.CompanyId) === companyId,
  );
  ...
  return (
    <div className="company-selector">
      ...
      <div className="company-selector-control">
        <span className="company-selector-mark" aria-hidden="true">
          ◆
        </span>
        <select
          value={companyId}
      ...
      {selectedCompany && (
        <span className="company-selector-status">● AKTİF</span>
      )}
    </div>
  );
};
