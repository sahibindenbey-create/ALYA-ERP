...
export default function ReceteYonetimPage() {
  ...
  const reset = () => { setForm({ ...emptyForm, receteKodu: newCode() }); setItems([]); setOperations([]); setSelected(null); setCost(null); setItem(emptyItem); setOp(emptyOp); };
  ...
  const save = async () => {
    ...
    try {
      await axios.post(`${API_URL}/recete-yonetim`, { form, items, operations });
      alert('Reçete başarıyla kaydedildi.');
      await load(); reset();
    } catch (e) { alert(e.response?.data?.error || 'Reçete kaydedilemedi.'); }
  };

  const openRecipe = async (r) => {
    try {
      const { data } = await axios.get(`${API_URL}/recete-yonetim/${r.ReceteId}`);
      setSelected(data); setForm({ receteKodu:data.ReceteKodu||'', receteAdi:data.ReceteAdi||'', mamulUrunId:data.MamulUrunId||'', mamulAdi:data.MamulAdi||'', aciklama:data.Aciklama||'', versiyon:data.Versiyon||1, uretimBirimi:data.UretimBirimi||'Adet', durum:data.Durum||'Aktif' });
  ...
  return <div className="recete-v2">
    <div className="recete-v2-title"><div><span>ÜRETİM</span><h1>Reçete Yönetimi</h1><p>BOM, üretim rotası ve güncel hammadde maliyeti</p></div><button className="rv2-primary" onClick={reset}>+ Yeni Reçete</button></div>
    ...
    <div className="rv2-layout">
      <aside className="rv2-list">
        ...
        <div className="rv2-scroll">{loading ? <div className="rv2-empty">Yükleniyor...</div> : filtered.map(r=><button className={`rv2-recipe ${selected?.ReceteId===r.ReceteId?'active':''}`} key={r.ReceteId} onClick={()=>openRecipe(r)}><b>{r.ReceteKodu}</b><span>{r.MamulAdi}</span><small>Rev. {r.Versiyon || 1} · {r.Durum || 'Aktif'}</small></button>)}{!filtered.length && <div className="rv2-empty">Reçete bulunamadı.</div>}</div>
      </aside>
      ...
      <main className="rv2-main">
        ...
        <section className="rv2-card">
          ...
          <div className="rv2-table-wrap"><table><thead><tr><th>#</th><th>Malzeme</th><th>Miktar</th><th>Birim</th><th>Fire</th><th>İstasyon</th><th></th></tr></thead><tbody>{items.map((x,i)=><tr key={x.id}><td>{i+1}</td><td><b>{x.hammaddeAdi}</b></td><td>{x.miktar}</td><td>{x.birim}</td><td>{Number(x.fireOrani||0).toFixed(2)}%</td><td>{x.istasyon||'—'}</td><td><button className="rv2-del" onClick={()=>setItems(v=>v.filter(y=>y.id!==x.id))}>Sil</button></td></tr>)}{!items.length&&<tr><td colSpan="7" className="rv2-empty">Henüz malzeme eklenmedi.</td></tr>}</tbody></table></div>
        </section>
        ...
        <section className="rv2-card">
          ...
          <div className="rv2-route">{operations.map((x,i)=><div className="rv2-op-chip" key={x.id}><strong>{i+1}</strong><span>{x.istasyonAdi}</span>{x.islemAdi&&<small>{x.islemAdi}</small>}{x.tahminiSureDk&&<em>{x.tahminiSureDk} dk</em>}<button onClick={()=>setOperations(v=>v.filter(y=>y.id!==x.id))}>×</button></div>)}</div>
        </section>
        ...
        <div className="rv2-actions"><button className="rv2-secondary" onClick={reset}>Temizle</button><button className="rv2-primary" onClick={save}>✓ Reçeteyi Kaydet</button></div>
      </main>
    </div>
  </div>;
}
