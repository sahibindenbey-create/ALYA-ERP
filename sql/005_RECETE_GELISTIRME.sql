...
GO
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name='IX_Receteler_Company_Mamul_Aktif' AND object_id=OBJECT_ID('dbo.Receteler')) CREATE INDEX IX_Receteler_Company_Mamul_Aktif ON dbo.Receteler(CompanyId,MamulUrunId,IsActive,Versiyon DESC);
GO
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name='IX_ReceteDetay_Company_Recete_Sira' AND object_id=OBJECT_ID('dbo.ReceteDetay')) CREATE INDEX IX_ReceteDetay_Company_Recete_Sira ON dbo.ReceteDetay(CompanyId,ReceteId,SiraNo);
GO
...
BEGIN
    ...
    BEGIN
        ...
        UPDATE ri SET CompanyId=r.CompanyId FROM dbo.ReceteIstasyon ri INNER JOIN dbo.Receteler r ON r.ReceteId=ri.ReceteId WHERE ri.CompanyId IS NULL;
...
IF OBJECT_ID('dbo.ReceteIstasyon','U') IS NOT NULL
BEGIN
    IF NOT EXISTS (SELECT 1 FROM sys.security_predicates p WHERE p.object_id=OBJECT_ID(N'dbo.SecurityPolicy_CompanyIsolation') AND p.target_object_id=OBJECT_ID(N'dbo.ReceteIstasyon') AND p.predicate_type=0)
        ALTER SECURITY POLICY dbo.SecurityPolicy_CompanyIsolation ADD FILTER PREDICATE dbo.fn_CompanyIsolationPredicate(CompanyId) ON dbo.ReceteIstasyon;
    IF NOT EXISTS (SELECT 1 FROM sys.security_predicates p WHERE p.object_id=OBJECT_ID(N'dbo.SecurityPolicy_CompanyIsolation') AND p.target_object_id=OBJECT_ID(N'dbo.ReceteIstasyon') AND p.predicate_type=1 AND p.operation=1)
        ALTER SECURITY POLICY dbo.SecurityPolicy_CompanyIsolation ADD BLOCK PREDICATE dbo.fn_CompanyIsolationPredicate(CompanyId) ON dbo.ReceteIstasyon AFTER INSERT;
    IF NOT EXISTS (SELECT 1 FROM sys.security_predicates p WHERE p.object_id=OBJECT_ID(N'dbo.SecurityPolicy_CompanyIsolation') AND p.target_object_id=OBJECT_ID(N'dbo.ReceteIstasyon') AND p.predicate_type=1 AND p.operation=2)
        ALTER SECURITY POLICY dbo.SecurityPolicy_CompanyIsolation ADD BLOCK PREDICATE dbo.fn_CompanyIsolationPredicate(CompanyId) ON dbo.ReceteIstasyon AFTER UPDATE;
END;
GO
