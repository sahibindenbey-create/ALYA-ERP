...
const registerKolaybi = ({ app, poolPromise, sql }) => {
  ...
  const companyIdFromRequest = (req) => {
    ...
    // Merkezi company-context-hook tarafından oluşturulan context
    // varsa öncelik her zaman seçili şirkettedir.
    const contextCompanyId =
      Number(storage.getStore()?.companyId);

    if ([1, 2, 3].includes(contextCompanyId)) {
    ...
    // Hiç şirket bilgisi gönderilmemiş eski çağrılar için varsayılan 1.
    if (
    ...
    if (![1, 2, 3].includes(id)) {
      const error = new Error('Geçersiz CompanyId');
      error.status = 400;
      throw error;
    }
  ...
  async function getValidToken(pool, companyId = 1) {
    ...
    console.log(`[KolayBi][Şirket ${companyId}] Yeni access token alınıyor...`);
  ...
  async function saveSyncRecord(pool, companyId, entityType, row) {
    ...
    if (!externalId || !entityId) {
      return { created: 0, updated: 0, skipped: 1 };
    }

    const direction = 'KOLAYBI_TO_ERP';
    const status = 'BAŞARILI';
    ...
    if (existing.recordset.length > 0) {
      await pool.request()
        ...
        .input('Direction', sql.NVarChar(100), direction)
        .input('Status', sql.NVarChar(100), status)
        .input('ErrorMessage', sql.NVarChar(2000), null)
        .query(`
  ...
  async function saveRaw(pool, companyId, entityType, rows) {
    ...
    console.log(`[KolayBi][Şirket ${companyId}] ${entityType}: ${list.length} kayıt işlenecek.`);
  ...
  async function syncCompany(companyId) {
    ...
    if (running.has(companyId)) {
      console.log(`[KolayBi][Şirket ${companyId}] Senkronizasyon zaten çalışıyor, atlandı.`);
    ...
    console.log('');
    console.log('======================================================');
    console.log(`[KolayBi] ŞİRKET ${companyId} SENKRONİZASYON BAŞLADI`);
    console.log('======================================================');
    ...
    try {
      ...
      for (const [type, endpoint, params] of jobs) {
        console.log(`[KolayBi][Şirket ${companyId}] ${type} senkronizasyonu başlıyor...`);
        ...
        try {
          ...
          console.error(`[KolayBi][Şirket ${companyId}] ${message}`);
        }
      }

      const finalStatus = errors > 0 ? 'KISMI_HATA' : 'BAŞARILI';
      const finalMessage = messages.length > 0 ? messages.join(' | ').slice(0, 1000) : null;
      ...
      console.log('');
      console.log(`[KolayBi][Şirket ${companyId}] SENKRONİZASYON TAMAMLANDI`);
      console.log(`Başlangıç  : ${started.toLocaleString('tr-TR')}`);
      console.log(`Bitiş      : ${finished.toLocaleString('tr-TR')}`);
      console.log(`Yeni       : ${created}`);
      console.log(`Güncellenen: ${updated}`);
      console.log(`Atlanan    : ${skipped}`);
      console.log(`Hata       : ${errors}`);
      console.log(`Durum      : ${finalStatus}`);
      ...
      return { companyId, created, updated, skipped, errors, status: finalStatus, messages };
    } finally {
  ...
  app.get('/api/sirketler', async (req, res) => {
    try {
      ...
      console.error('Şirketler alınamadı:', err.message);
  ...
  app.get('/api/kolaybi/ayarlar', async (req, res) => {
    try {
      ...
      console.error('KolayBi ayarları alınamadı:', err.message);
  ...
  app.put('/api/kolaybi/ayarlar', async (req, res) => {
    try {
      ...
      const request = pool.request()
        ...
        .input('KolaybiCompanyId', sql.NVarChar, KolaybiCompanyId ? String(KolaybiCompanyId) : null);
      ...
      if (!result.rowsAffected || result.rowsAffected[0] === 0) {
        return res.status(404).json({
          error: `Şirket ${companyId} için KolayBi ayar kaydı bulunamadı.`
        });
      }

      res.json({
        success: true,
        message: `Şirket ${companyId} KolayBi ayarları güncellendi.`
      });
    } catch (err) {
      console.error('KolayBi ayar güncelleme hatası:', err.message);
  ...
  app.post('/api/kolaybi/test-baglanti', async (req, res) => {
    ...
    try {
      ...
      const companies = await kolaybiRequest(
        pool,
        companyId,
      ...
      res.json({
        success: true,
        message: 'Bağlantı başarılı, access token alındı.',
        CompanyId: companyId,
        KolaybiSirketleri: companies?.data || []
      });
    } catch (err) {
      console.error(
        `[KolayBi][Şirket ${companyId ?? 'bilinmiyor'}] Bağlantı testi hatası:`,
        err.message
      );
  ...
  app.get('/api/kolaybi/sirketler', async (req, res) => {
    try {
      const pool = await poolPromise;
      const companyId = companyIdFromRequest(req);
      const result = await kolaybiRequest(pool, companyId, '/kolaybi/v1/companies');
      res.json(result);
    } catch (err) {
      res.status(err.status || 500).json({ error: err.message });
    }
  ...
  app.get('/api/kolaybi/faturalar-onizleme', async (req, res) => {
    try {
      ...
      const params = type
        ? { type, has_products: true }
        : { has_products: true };

      const result = await kolaybiRequest(
        pool,
        companyId,
        '/kolaybi/v1/invoices',
        params
  ...
  app.post('/api/kolaybi/senkronize-et', async (req, res) => {
    ...
    try {
      ...
      console.error(
        `[KolayBi][Şirket ${companyId ?? 'bilinmiyor'}] Manuel senkronizasyon hatası:`,
        err.message
      );
      res.status(err.status || 500).json({ error: err.message });
    }
  ...
  app.get('/api/kolaybi/senkronizasyon-durumu', async (req, res) => {
    try {
      ...
      res.json({
        companyId,
        running: running.has(companyId),
        ayar: ayar ? { SonSenkronTarihi: ayar.SonSenkronTarihi } : null,
        sonLog: log.recordset[0] || null
      });
    } catch (err) {
      console.error('KolayBi senkronizasyon durumu hatası:', err.message);
      res.status(err.status || 500).json({ error: err.message });
    }
  });
    ...
    try {
      ...
      for (const row of companies.recordset) {
        ...
        storage.run(
          { companyId },
          () => {
            syncCompany(companyId).catch(err => {
              console.error(
