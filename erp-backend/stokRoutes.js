...
module.exports = function registerStokRoutes(app) {
  ...
  app.get('/api/stok/hareketler', async (req, res) => {
    try {
      ...
      const result = await request.query(`
        ...
        WHERE (@UrunId IS NULL OR h.UrunId=@UrunId)
  ...
  app.post('/api/stok/hareket', async (req, res) => {
    ...
    try {
      ...
      request.input('UrunId', sql.Int, urunId);
      const urun = await request.query(`SELECT UrunId, StokMiktari, Tur FROM dbo.Urunler WHERE UrunId=@UrunId AND IsActive=1`);
      if (!urun.recordset[0]) throw new Error('Ürün bulunamadı veya pasif.');
      ...
      update.input('UrunId', sql.Int, urunId);
      update.input('SonrakiStok', sql.Decimal(18,4), sonraki);
      await update.query(`UPDATE dbo.Urunler SET StokMiktari=@SonrakiStok, UpdatedAt=SYSDATETIME() WHERE UrunId=@UrunId`);
      ...
      const result = await insert.query(`
        ...
        VALUES
          (TRY_CONVERT(INT,SESSION_CONTEXT(N'CompanyId')),@UrunId,@Depo,@HareketTipi,@Miktar,@OncekiStok,@SonrakiStok,@ReferansTipi,@ReferansId,@Aciklama);
