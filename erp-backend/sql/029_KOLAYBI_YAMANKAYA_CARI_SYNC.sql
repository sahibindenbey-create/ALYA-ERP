...
EXEC sys.sp_executesql N'CREATE TABLE dbo.KolaybiCariEslemeleri(
 ...
 KolaybiAssociateId NVARCHAR(100) NOT NULL,
 CariId INT NULL,
 KolaybiCode NVARCHAR(100) NULL,
 KolaybiName NVARCHAR(250) NOT NULL,
...
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.KolaybiCariEslemeleri') AND name=N'IX_KolaybiCariEslemeleri_Cari')
 EXEC sys.sp_executesql N'CREATE INDEX IX_KolaybiCariEslemeleri_Cari ON dbo.KolaybiCariEslemeleri(CompanyId,CariId) INCLUDE(KolaybiAssociateId,SyncStatus,LastSyncedAt);';
...
BEGIN
 ...
 IF NOT EXISTS(SELECT 1 FROM sys.security_predicates WHERE object_id=OBJECT_ID(N'dbo.SecurityPolicy_CompanyIsolation') AND target_object_id=OBJECT_ID(N'dbo.KolaybiCariEslemeleri') AND predicate_type=1 AND operation=4)
