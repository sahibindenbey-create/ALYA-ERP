/*
  ...
  Güvenlik kuralları:
    ...
    - Mamul ürünü şirket içinde koduyla tekil eşleşmiyorsa import edilmez.
    - Hammadde ürünü bulunmayan / birden fazla eşleşen kayıtlar import edilmez.
    - '?' içeren REVIEW satırları import edilmez.
    - Bir mamulün staging satırlarının tamamı güvenli eşleşmeden geçmeden reçete oluşturulmaz.
    - Mevcut dolu reçetenin BOM'u değiştirilmez.
    - RAW satırlarında "ALT REÇETE: AHDRYxxxx" gerçek AltReceteId'ye bağlanır.

  Varsayılan davranış RAPOR'dur. Gerçek import için @APPLY = 1 yapılmalıdır.
*/
...
(
    SELECT
        ...
        s.MamulKodu,
        CASE
            WHEN s.Durum = N'REVIEW' OR s.HammaddeAdi LIKE N'%?%' THEN 1
            ELSE 0
        END AS ReviewSatiri,
        CASE
            ...
            THEN
                ...
                (
                    SELECT 1
                    FROM dbo.Receteler ar
                    WHERE ar.CompanyId = @CompanyId
                      AND ar.IsActive = 1
                      AND UPPER(LTRIM(RTRIM(ar.ReceteKodu))) =
                          UPPER(LTRIM(RTRIM(REPLACE(s.HammaddeAdi, N'ALT REÇETE:', N''))))
                ) THEN 0 ELSE 1 END
            ELSE
                CASE
                    WHEN
                    (
                        SELECT COUNT(DISTINCT hu.UrunId)
                        FROM dbo.Urunler hu
                        WHERE hu.CompanyId = @CompanyId
                          AND
                          (
                              UPPER(LTRIM(RTRIM(hu.UrunKodu))) = UPPER(LTRIM(RTRIM(s.HammaddeAdi)))
...
(
    SELECT
        ...
        SUM(ReviewSatiri) AS ReviewSayisi,
        SUM(EslesmeyenSatir) AS EslesmeyenSatir
    FROM SatirKontrol
    GROUP BY MamulKodu
)
SELECT
...
/* =========================================================
   2) SADECE TAM VE GÜVENLİ BOM'LARI GERÇEK REÇETEYE AKTAR
   ========================================================= */
BEGIN TRANSACTION;
...
SELECT
    m.MamulKodu,
    m.MamulUrunId,
    MAX(COALESCE(NULLIF(s.MamulAdi, N''), u.UrunAdi)) AS MamulAdi
FROM
(
    SELECT
        s.MamulKodu,
...
JOIN dbo.Urunler u
  ON u.CompanyId = @CompanyId
 AND u.UrunId = m.MamulUrunId
JOIN dbo.BomImportStaging s
  ON s.CompanyId = @CompanyId
 AND s.MamulKodu = m.MamulKodu
WHERE m.MamulEslesme = 1
GROUP BY m.MamulKodu, m.MamulUrunId
HAVING
    SUM(CASE WHEN s.Durum = N'REVIEW' OR s.HammaddeAdi LIKE N'%?'% THEN 1 ELSE 0 END) = 0
    AND
    SUM
    (
        CASE
...
BEGIN
    ...
    BEGIN
        ...
        SET @ReceteId = CONVERT(INT, SCOPE_IDENTITY());
    END;

    /* Mevcut reçetenin BOM'u boşsa staging aktarılır; dolu BOM'a dokunulmaz. */
    IF NOT EXISTS
    (
        SELECT 1
        FROM dbo.ReceteDetay
        WHERE CompanyId = @CompanyId
          AND ReceteId = @ReceteId
    )
    BEGIN
        ...
        (
            ...
            ORDER BY
                CASE
                    WHEN UPPER(LTRIM(RTRIM(u.UrunKodu))) = UPPER(LTRIM(RTRIM(s.HammaddeAdi))) THEN 0
                    ELSE 1
                END,
                u.UrunId
        ) hu
        ...
        (
            ...
            WHERE r.CompanyId = @CompanyId
              AND r.IsActive = 1
              AND UPPER(LTRIM(RTRIM(r.ReceteKodu))) =
                  UPPER(LTRIM(RTRIM(REPLACE(s.HammaddeAdi, N'ALT REÇETE:', N''))))
            ORDER BY r.Versiyon DESC, r.ReceteId DESC
        ) ar
        WHERE s.CompanyId = @CompanyId
          AND s.MamulKodu = @MamulKodu;
    END;

    UPDATE s
       SET Durum = N'IMPORTED',
           Notlar = CONCAT
           (
               ISNULL(Notlar, N''),
               CASE WHEN ISNULL(Notlar, N'') = N'' THEN N'' ELSE N' | ' END,
               N'Reçeteye aktarıldı: ',
               CONVERT(NVARCHAR(20), @ReceteId)
           )
    FROM dbo.BomImportStaging s
    WHERE s.CompanyId = @CompanyId
      AND s.MamulKodu = @MamulKodu;
