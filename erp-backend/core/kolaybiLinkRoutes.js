...
module.exports = function registerKolaybiLinkRoutes(app, poolPromise, sql) {
  ...
  const onlyYamankaya = req => {
    if (req.companyId !== 2) throw Object.assign(new Error('İlişkilendirme yalnız Yamankaya için açıktır.'), { statusCode: 409 });
  };

  async function metrics(db, transaction) {
    const request = transaction ? new sql.Request(transaction) : db.request();
    const clearContext = transaction ? '' : "EXEC sys.sp_set_session_context @key=N'CompanyId',@value=NULL;";
    const query = await request.input('CompanyId', sql.Int, 2).query(`
      EXEC sys.sp_set_session_context @key=N'CompanyId',@value=@CompanyId;
      SELECT COUNT_BIG(*) Total,SUM(CASE WHEN f.CariId IS NOT NULL THEN 1 ELSE 0 END) Linked FROM dbo.KolaybiFaturaEslemeleri e JOIN dbo.Faturalar f ON f.CompanyId=e.CompanyId AND f.FaturaId=e.FaturaId WHERE e.CompanyId=@CompanyId;
      SELECT COUNT_BIG(*) Total,SUM(CASE WHEN d.UrunId IS NOT NULL THEN 1 ELSE 0 END) Linked FROM dbo.KolaybiFaturaEslemeleri e JOIN dbo.FaturaDetay d ON d.CompanyId=e.CompanyId AND d.FaturaId=e.FaturaId WHERE e.CompanyId=@CompanyId;
      SELECT COUNT_BIG(*) Total,SUM(CASE WHEN s.CariId IS NOT NULL THEN 1 ELSE 0 END) Linked FROM dbo.KolaybiBusinessMappings m JOIN dbo.Siparisler s ON m.CompanyId=s.CompanyId AND m.AlyaTable=N'Siparisler' AND m.AlyaId=s.SiparisId WHERE m.CompanyId=@CompanyId;
      SELECT COUNT_BIG(*) Total,SUM(CASE WHEN t.CariId IS NOT NULL THEN 1 ELSE 0 END) Linked FROM dbo.KolaybiBusinessMappings m JOIN dbo.Teklifler t ON m.CompanyId=t.CompanyId AND m.AlyaTable=N'Teklifler' AND m.AlyaId=t.TeklifId WHERE m.CompanyId=@CompanyId;
      SELECT COUNT_BIG(*) Total FROM dbo.CariDefterHareketleri WHERE CompanyId=@CompanyId AND BelgeTipi=N'KolayBiFatura';
      SELECT COUNT_BIG(*) Total FROM dbo.BelgeBaglantilari WHERE CompanyId=@CompanyId AND KaynakTip LIKE N'KolayBi%';
      ${clearContext}`);
  ...
  router.get('/plan', requirePermission('erp.read'), async (req,res) => {
    try { onlyYamankaya(req); res.json({ success:true, metrics:await metrics(await poolPromise) }); }
    catch (error) { res.status(error.statusCode || 500).json({ success:false, error:error.message }); }
  });

  router.post('/run', requirePermission('core.admin'), async (req,res) => {
    if (req.body?.confirmation !== 'YAMANKAYA_LINK_DATA') return res.status(400).json({ success:false, error:'İlişkilendirme onayı eksik.' });
    let transaction;
    try {
      onlyYamankaya(req);
      ...
      await new sql.Request(transaction).input('CompanyId',sql.Int,2).query("EXEC sys.sp_set_session_context @key=N'CompanyId',@value=@CompanyId;");
